...
Debtors and Creditor list will only be updated when a person in the person card panel list is selected. When a person is selected, PersonPanelSelectionChangedEvent will be handled by the UI and post an updateDebtorsList and updateCreditorsList event which will be handled by logic as shown by the code below.
...
* traveller who share many expenses with friends
...
* Major feature: Allow users to split their expenses using evenly, by units, and by percentage
* Minor feature: Calculate and display a net balance for every user based on all their transactions
* Minor feature: Ensure correct amount is entered for pay debt transaction. (E.g debt must exists, amount entered cannot be more than the debt owed)
* Minor feature: Calculate and display a net balance for every user based on all their transactions
* Minor feature: Ensure payer or payee(s) indicated in the transaction exist for every a add transaction command
...
|`* * *` |user |use a user-friendly interactive interface |enter input
...
A method of dividing the total cost of an expense by units and then assigning the specific amount of units (which can vary among users) owed by a user to that user

[[settle-balance]] Settle balance::
A feature that allows a user to record a payment that was made to another user to settle the debt owed
...
. A person can be added only if the name is different and is case sensitive.
...
. A transaction can be either payment or paydebt.
...
. A paydebt type transaction only allow one payer to one payee.
